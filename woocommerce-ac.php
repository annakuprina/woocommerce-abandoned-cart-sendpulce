<?php
/*
* Plugin Name: Abandoned Cart Lite for WooCommerce Custom
* Description: This plugin captures abandoned carts by logged-in users & send this data to SendPulse.
* Version: 99.0.0
* Author: FreeUA
* Author URI: http://freeua.agency/
...
require_once ("sendpulse-rest-api-php-master/src/ApiInterface.php");
require_once ("sendpulse-rest-api-php-master/src/ApiClient.php");
require_once ("sendpulse-rest-api-php-master/src/Storage/TokenStorageInterface.php");
require_once ("sendpulse-rest-api-php-master/src/Storage/FileStorage.php");
require_once ("sendpulse-rest-api-php-master/src/Storage/SessionStorage.php");
require_once ("sendpulse-rest-api-php-master/src/Storage/MemcachedStorage.php");
require_once ("sendpulse-rest-api-php-master/src/Storage/MemcacheStorage.php");

use Sendpulse\RestApi\ApiClient;
use Sendpulse\RestApi\Storage\FileStorage;
define('API_USER_ID', '5150a9fb63424582e78e35f7a05d5d35');
define('API_SECRET', '6ef1bff72b2910db2140def7f98c6e0d');
...
if ( ! class_exists( 'woocommerce_abandon_cart_lite' ) ) {
    ...
    class woocommerce_abandon_cart_lite {
        ...
        public function __construct() {
            ...
            add_action('woocommerce_thankyou', array( &$this,  'wcal_purchase_sendpulse' ), 10, 1 );
            add_action( 'woocommerce_payment_complete', array(&$this, 'wcal_purchase_sendpulse' ), 10 );
        ...
        function wcal_initialize_plugin_options() {
            ...
            add_settings_field(
                'ac_lite_cart_abandoned_abandon_cart_url',
                __( 'Sendpulse Abandon Cart REST API POST url', 'woocommerce-abandoned-cart' ),
                array( $this, 'ac_lite_cart_abandoned_abandon_cart_url_callback' ),
                'woocommerce_ac_page',
                'ac_lite_general_settings_section',
                array( __( 'REST API POST url for sendpulse event Abandon Cart.', 'woocommerce-abandoned-cart' ) )
            );

            add_settings_field(
                'ac_lite_cart_abandoned_purchase_url',
                __( 'Sendpulse Purchase REST API POST url', 'woocommerce-abandoned-cart' ),
                array( $this, 'ac_lite_cart_abandoned_purchase_url_callback' ),
                'woocommerce_ac_page',
                'ac_lite_general_settings_section',
                array( __( 'REST API POST url for sendpulse event Purchase.', 'woocommerce-abandoned-cart' ) )
            );
        ...
        function ac_lite_cart_abandoned_abandon_cart_url_callback( $args ) {
            // First, we read the option
            $cart_abandoned_cart_url = get_option( 'ac_lite_cart_abandoned_abandon_cart_url' );
            // Next, we update the name attribute to access this element's ID in the context of the display options array
            // We also access the show_header element of the options collection in the call to the checked() helper function
            $html = '<input type="text" id="ac_lite_cart_abandoned_abandon_cart_url" name="ac_lite_cart_abandoned_abandon_cart_url" value="'. $cart_abandoned_cart_url .'" style="width:60%;"/>';
            $html .= '<label for="ac_lite_cart_abandoned_abandon_cart_url"> '  . $args[0] . '</label>';
            echo $html;
        }

        function ac_lite_cart_abandoned_purchase_url_callback( $args ) {
            // First, we read the option
            $purchase_url = get_option( 'ac_lite_cart_abandoned_purchase_url' );
            // Next, we update the name attribute to access this element's ID in the context of the display options array
            // We also access the show_header element of the options collection in the call to the checked() helper function
            $html = '<input type="text" id="ac_lite_cart_abandoned_purchase_url" name="ac_lite_cart_abandoned_purchase_url" value="'. $purchase_url .'" style="width:60%;"/>';
            $html .= '<label for="ac_lite_cart_abandoned_purchase_url"> '  . $args[0] . '</label>';
            echo $html;
        }
        ...
        function wcal_store_cart_timestamp() {
            ...
            if ( is_user_logged_in() ) {
                ...
                $user_email_sendpulse = $user_info->user_email;
                $user_name_sendpulse = $user_info->display_name;
                $user_phone_sendpulse = get_user_meta( $user_id, 'billing_phone', true );
                ...
                if( $gdpr_consent ) {
                    ...
                    if ( 0 == count( $results ) ) {
                        ...
                        if( '' !== $cart_info_meta && '{"cart":[]}' != $cart_info_meta && '""' !== $cart_info_meta ) {
                            ...
                            $createEvent = array(
                                'email' => $user_email_sendpulse,
                                'phone' => $user_phone_sendpulse,                             
                                'name' => $user_name_sendpulse,
                            );

                            $SPApiClient->createEvent($createEvent);
...
?>
        ...
        function wcal_purchase_sendpulse( $order_id ){
            $SPApiClient = new ApiClient(API_USER_ID, API_SECRET, new FileStorage());
            $order = wc_get_order( $order_id );
            $orderstat = $order->get_status();
            if ($orderstat == 'processing') {
                $user_Purchase_email_sendpulse = $order->billing_email ;
                $user_Purchase_phone_sendpulse = $order->billing_phone;
                $user_Purchase_name_sendpulse = $order->billing_first_name . ' ' .$order->billing_last_name;

                $createPurchase = array(
                  'email' => $user_Purchase_email_sendpulse,
                  'phone' => $user_Purchase_phone_sendpulse,                             
                  'name' => $user_Purchase_name_sendpulse,
                );

                $SPApiClient->createPurchase($createPurchase);

...
if ( !class_exists( 'woocommerce_abandon_cart_cron' ) ) {
    ...
    class woocommerce_abandon_cart_cron { 
        ...
        function wcal_send_email_notification() {
            ...
            $hour_seconds = 1800;   // 30 * 30
            $cart_abandon_cut_off_time = $cart_settings * 30;
            $time_to_send_template_after = 1 * $hour_seconds;
            $carts               = $this->wcal_get_carts( $time_to_send_template_after, $cart_abandon_cut_off_time, '1' );
            foreach ( $carts as $key => $value ) {
                $query_guest       = "SELECT billing_first_name, billing_last_name, email_id, phone FROM `".$wpdb->prefix."ac_guest_abandoned_cart_history_lite`
                                                        WHERE id = %d";
                ...
                $billing_phone = $results_guest[0]->phone;
                $guest_full_name = $billing_first_name . ' ' . $billing_last_name;
                $guest_email = $results_guest[0]->email_id;
                ...
                $update_sent_history['cart_ignored'] = '1';
                $update_sent_history['unsubscribe_link'] = '1';
                $wpdb->update( 
